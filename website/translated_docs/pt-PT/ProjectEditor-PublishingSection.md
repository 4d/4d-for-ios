...
:::tip NOTE * O simulador funciona localmente (127.0.0.1 ou localhost). * Se a URL de produção estiver definida, tanto HTTP quanto HTTPS são aceitas. Senão, o endereço IP é usado. * Se tanto HTTP quanto HTTPS estiverem ativados, HTTP é usado. :::
...
Se quiser a habilidade de manualmente validar o primeiro login para cada sessão de usuário, deve mudar o estado padrão "accepted" para "pending" adicionando ```$response.verify:=True``` ao método de banco de dados [*On Mobile App Authentication*](https://doc.4d.com/4Dv17R3/4D/17-R3/On-Mobile-App-Authentication-database-method.301-3906587.en.html).
