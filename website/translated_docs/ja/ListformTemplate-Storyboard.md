...
このレッスンでは，リスト画面のカスタムテンプレートを作成する方法を紹介しました。 素材プロジェクトから始めて，シンプルなカスタムテンプレートが簡単に自作できる，という手応えが得られたのではないでしょうか。 しかし，知るべきことは，これだけではありません！ 続くレッスンでは，詳細フォームのカスタムテンプレートを作成する方法が取り上げられます。お楽しみに！ Click on **Final Project** below to download the completed template list folder.
